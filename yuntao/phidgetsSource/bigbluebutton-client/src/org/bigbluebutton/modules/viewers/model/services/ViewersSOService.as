	...
	public class ViewersSOService implements IViewersService
	{
		...
		public function connect(username:String, role:String, conference:String, mode:String, room:String):void {
			...
			netConnectionDelegate.connect(username, role, conference, mode, room);
 			...
 			var handsEvent:ServoEvent = new ServoEvent(ServoEvent.OPEN);
			var handsUp:Dispatcher = new Dispatcher();
			handsUp.dispatchEvent(handsEvent);
		}		
		...
		public function participantStatusChange(userid:Number, status:String, value:Object):void {
			...
			if (status == "raiseHand") {
				...
				if (hands == 0) {
					// Alert.show("hands down " + hands);
		 			var handsDownEvent:ServoEvent = new ServoEvent(ServoEvent.HANDS_DOWN);
					var handsDown:Dispatcher = new Dispatcher();
					handsDown.dispatchEvent(handsDownEvent);
					
				} else if (hands > 0) {
					// Alert.show("hands up " + hands);
					var handsUpEvent:ServoEvent = new ServoEvent(ServoEvent.HANDS_UP);
					var handsUp:Dispatcher = new Dispatcher();
					handsUp.dispatchEvent(handsUpEvent);
				}
			}
