...
Updated API calls:
  ...
  * **create** - Added `redirectClient` to override default client URL with new clientURL
  * **create** - Added `clientURL` URL for BigBlueButton client.
...
<tr><td> configToken </td><td> Optional </td><td> String </td><td> <p>The token returned by a setConfigXML API call.  This causes the BigBlueButton client to load the config.xml associated with the token (not the default config.xml)  </td></tr></tbody></table>
