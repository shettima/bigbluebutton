## Rails ##
  * http://robzon.aenima.pl/2007/10/ubuntu-710-rails-gedit-and.html
  * http://guides.rubyonrails.org/getting_started.html

  * [Loading ActionScript 2.0 content in Flex](http://www.adobe.com/cfusion/communityengine/index.cfm?event=showdetails&productId=2&postId=5161)
  * [PDF2SWF - Flash 9 ActionScript 3 export](http://www.mail-archive.com/swftools-common@nongnu.org/msg01107.html)
  * [Retrieving browser and server "State" information in Flex](http://www.adobe.com/cfusion/communityengine/index.cfm?event=showDetails&loc=en_US&postId=2001&productId=2)
  * [JSecurity Plugin](http://docs.codehaus.org/display/GRAILS/JSecurity+Plugin)
  * [JSecurity Plugin - Quick Start](http://docs.codehaus.org/display/GRAILS/JSecurity+Plugin+-+Quick+Start)
  * [REST in Spring without using Spring Webservices](http://bandaidprogramming.blogspot.com/2008/05/rest-in-spring-without-using-spring.html)
  * [Generating unique IDs](http://www.javapractices.com/topic/TopicAction.do?Id=56)
  * http://code.google.com/p/zipwire-rest/wiki/HighriseWalkthrough
  * [Screen sharing in Flash video chat applications](http://www.avchat.net/blog/?p=19)
  * http://www.wizhelp.com/flashlight-vnc/features.html
  * http://www.ioncannon.net/php/110/using-flash-video-metadata-to-display-annotations/


## S5 ##
  * [to S5 • Reloaded](http://www.netzgesta.de/S5/)
  * [Slide Show (S9)](http://slideshow.rubyforge.org/)
  * http://golem.ph.utexas.edu/instiki/show/S5+Test+(flower)
  * http://www.instiki.org
  * http://lawver.net/archive/2005/03/18/h15_s5_instiki_happy_geek.php
  * http://code.google.com/p/flex-iframe/
  * [An IFrame which you can embed into Flex applications to show an html page (flex-iframe)](http://code.google.com/p/flex-iframe/)
  * [Embedding browser in Flex](http://www.deitte.com/IFrameDemo3/IFrameDemo.html)

## Markdown ##
  * http://code.google.com/p/markdownj/
  * [Markdown Testbed](http://babelmark.bobtfish.net/)
  * [Showdown - a JavaScript port of Markdown](http://attacklab.net/showdown/)
  * [Generate a PDF book with groovy and iText](http://rghosh.free.fr/essays/txttopdf.html)
  * [Example: A Wiki parser](http://livedocs.adobe.com/flex/3/html/help.html?content=12_Using_Regular_Expressions_12.html)
  * [java-wikipedia-parser](http://code.google.com/p/java-wikipedia-parser/wiki/Introduction)

## textile ##
  * https://textile-j.dev.java.net/usage.html


## Phone Applet ##
  * [Signing Java Applet](http://forums.sun.com/thread.jspa?threadID=174214)
  * [Corraleta SDK](http://www.mexuar.com/files/corraleta_sdk.rar)
  * [netscape.javascript.JSObject](http://www.jguru.com/faq/view.jsp?EID=272810)
  * http://www.yosd.at/corraleta/

## Red5 ##
  * http://jira.red5.org/confluence/display/docs/Scopes+and+Contexts
  * http://jira.red5.org/confluence/display/docs/Chapter+5.+Migration+Guide
  * http://jira.red5.org/confluence/display/docs/Chapter+11.+Create+new+applications+in+Red5
  * http://jira.red5.org/confluence/display/docs/Table+Of+Contents
  * http://jira.red5.org/confluence/display/docs/Home
  * http://jira.red5.org/confluence/dashboard.action
  * http://osflash.org/red5/help
  * http://osflash.org/pipermail/red5_osflash.org/2006-May/003757.html
  * http://www.dokeos.com/wiki/index.php/Installing_videoconference
  * http://dl.fancycode.com/red5/0.6.3/war/Red5%20War.pdf
  * http://gregoire.org/
  * http://www.orbited.org/
  * http://jira.red5.org/confluence/display/docs/FAQ
  * [API](http://dl.fancycode.com/red5/api/)
  * http://jira.red5.org/confluence/display/docs/Red5+Developer+Tips
  * http://jira.red5.org/confluence/display/docs/Chapter+20.+Testing+Red5
  * http://jira.red5.org/confluence/display/codecs/Data+Type+Mappings
  * http://jira.red5.org/confluence/display/appserver/Benchmark+Profiler
  * http://jira.red5.org/confluence/display/docs/Logging+Setup
  * http://red5wiki.com/wiki/SteamStream
  * http://www.technogumbo.com/projects/red5-tuning-application/index.php
  * http://gregoire.org/2009/04/07/on-demand-room-scope-creation/

# Some links #
  * http://code.google.com/p/smartypants-ioc/
  * http://www.pranaframework.org/
  * http://code.google.com/p/swizframework/
  * http://www.oreillynet.com/ruby/blog/2006/07/deploying_rails_with_pound_in.html

  * http://wrapper.tanukisoftware.org/doc/english/download.jsp

  * http://www.javalobby.org/forums/thread.jspa?forumID=61&threadID=15711

  * http://algorithmist.wordpress.com/category/papervision-3d/
  * http://www.google.ca/search?client=firefox-a&rls=org.mozilla%3Aen-US%3Aofficial&channel=s&hl=en&q=papervision+flex&meta=&btnG=Google+Search
  * http://www.papervision3dblog.com/

  * http://www.adobe.com/devnet/flash/articles/flv_encoding.html
  * http://fms.startpagina.be/
  * http://88.152.22.77/Serendipity/

# Flash #
  * http://blog.flexcommunity.net/?m=200802
  * http://www.gotoandlearn.com/

  * [Preloader](http://www.pathf.com/blogs/2008/08/custom-flex-3-lightweight-preloader-with-source-code/)

# TESTING #
  * [Flex unit testing and continuous integration: Part 1 of 2](http://www.blackpepper.co.uk/black-pepper-blog/Flex-unit-testing-and-continuous-integration.html)
  * [Hudson](https://hudson.dev.java.net/)

# Thumbnails #
  * http://www.quietlyscheming.com/blog/components/fisheye-component/

  * [Converting PDF to Flashpaper](http://www.darronschall.com/weblog/2006/12/batch-convert-pdf-files-to-flashpaper-2.cfm)

# Wikis #
  * http://www.xwiki.org/xwiki/bin/view/Main/

# Creating thumbnails using ImageMagick #
  * http://blog.prashanthellina.com/2008/02/03/create-pdf-thumbnails-using-imagemagick-on-linux/
  * http://blog.robfelty.com/2008/03/11/convert-pdf-to-png-with-imagemagick/

# Flex #
  * http://flashcommander.org/remotefilelist
  * http://examples.adobe.com/flex3/componentexplorer/explorer.html
  * http://examples.adobe.com/flex3/consulting/styleexplorer/Flex3StyleExplorer.html#
  * http://www.insideria.com/2008/12/the-design-of-a-rich-spatial-f.html
  * [Flex and the Enterprise: Making Architectural Decisions](http://danorlando.com/?p=106)
  * http://www.insideria.com/2008/02/flex-graphics-tricks-part-2-bl.html
  * http://www.libspark.org/wiki/saqoosha/FLARToolKit/en

# Grails #
  * http://thevirtualmachine.wordpress.com/2008/11/07/glassfish-v3-and-grails-support/
  * http://www.grailstutorials.com/tagging/list/deploy?selectedTag=deploy
  * [Poster](https://addons.mozilla.org/en-US/firefox/addon/2691)
  * http://www.ibm.com/developerworks/views/java/libraryview.jsp?search_by=mastering+grails
  * [Securing a Grails Application with Acegi Security](http://www.infoq.com/articles/grails-acegi-integration)
  * http://grails.org/AcegiSecurity+Plugin


# Augmented Reality #
  * http://drawlogic.com/2008/11/17/as3-augmented-reality-in-flash-and-papervision-3d-flartoolkit/
  * [Papervision - Augmented Reality](http://www.boffswana.com/news/?p=392)

## TOmcat ##
  * https://help.ubuntu.com/community/ApacheTomcat5
  * http://www.klawitter.de/tomcat80.html
  * http://www.owasp.org/index.php/Securing_tomcat#Using_Port_80
  * http://www.mkyong.com/tomcat/tomcat-javasecurityaccesscontrolexception-access-denied-loggingproperties-read/
  * http://tomcat.apache.org/tomcat-5.5-doc/logging.html


## Asterisk ##
  * http://godson.in/how-to-install-asterisk-on-ubuntu
  * [Recording](http://www.orecx.com/web/)

## Fitnesse ##
  * http://hartsock.blogspot.com/2007/08/getting-fit-and-groovy.html
  * http://www.grails-exchange.com/files/Heritier-Robini-GrailsInAgileEntrepriseEnvironment.pdf
  * [Tutorial showing how to test Grails application with FitNesse](http://forge.octo.com/confluence/display/ATG/Tutorial)
  * http://www.fitnesse.info/fixturegallery

## Agile ##
  * http://www.infoq.com/minibooks/domain-driven-design-quickly
  * http://www.domaindrivendesign.org/books/index.html
  * http://www.versionone.com/Resources/AgileDevelopment.asp
  * http://agilesoftwaredevelopment.com/blog/vaibhav/good-user-story-invest
  * http://pmstudent.com/scrum-in-under-10-minutes/
  * http://www.mountaingoatsoftware.com/presentations/42-effective-user-stories-for-agile-requirements

## Tools ##
  * http://mmonit.com/
  * http://reductivelabs.com/products/puppet/
  * http://wiki.smartfrog.org/wiki/display/sf/SmartFrog+Home
  * http://www.opennms.org/index.php/Main_Page

## JMX ##
  * http://marxsoftware.blogspot.com/2008/06/remote-jmx-with-and-without-spring-and.html

## Virtualized Dev Environment ##
  * http://tomasvarsavsky.com/2008/02/25/virtualised-ruby-on-rails-development-environment-for-dummies/

## LDAP ##
  * http://www.zytrax.com/books/ldap/

## REST ##
  * http://www.onjava.com/pub/a/onjava/2007/11/07/introduction-to-amazon-s3-with-java-and-rest.html
  * http://awalkingcity.com/blog/2008/03/07/basic-authentication-and-grails/

## Content Repository ##
  * http://www.infoq.com/articles/spring-modules-jcr
  * http://www.ibm.com/developerworks/java/library/j-jcr/
  * http://www.onjava.com/pub/a/onjava/2006/10/04/what-is-java-content-repository.html
  * http://lorand.somogyi.name/2008/11/jackrabbit-with-spring/

## Tunneling ##
  * http://www.devx.com/Java/Article/17679/1954


# Slideshow #
  * http://meyerweb.com/eric/tools/s5/

# FSM #
http://smc.sourceforge.net/SmcHome.htm
http://unimod.sourceforge.net/intro.html

# Flash Player #
  * http://www.jeroenwijering.com/?item=JW_FLV_Media_Player
  * http://www.flashinsider.com/2006/07/26/how-to-create-your-own-youtube-site/
  * http://www.howforge.com/open-source-youtube
  * http://pandastream.com/
  * http://phpmotion.com/
  * http://corp.kaltura.com/
  * http://www.moviemasher.com/
  * http://www.metavid.org

# Papervision #
  * http://papervision2.com/tutorial-list/
  * http://pv3d.org/
  * http://www.madvertices.com/2008/01/papervision-3d-training-videos.html
  * http://papervision3d-fr.com/
  * http://www.dehash.com/?p=152
  * http://blog.tartiflop.com/
  * http://blog.tartiflop.com/2008/09/first-steps-in-papervision3d-part-8-movie-materials/
  * http://dev.papervision3d.org/
  * http://www.blender.org/

# Amazon EC2 #
  * http://www.techexperiment.com/2008/09/24/how-amazon-s3-and-ec2-can-cut-your-hosting-bills-in-half/
  * http://scottpenberthy.com/2008/09/24/amazon-aws-vs-rackspace-and-akamai/
  * http://www.mikebrittain.com/blog/2008/07/19/web-hosting-on-ec2/
  * http://ronaldlewis.com/asterisk-pbx-on-amazon-ec2-how-to-guide-almost-complete/
  * http://www.scribd.com/doc/3905321/PREVIEW-CloudCrunch-Howto-Asterisk-PBX-and-Amazon-EC2
  * http://www.donaldflood.com/2007/07/21/installing-asterisk-on-amazon-ec2/
  * http://www.causecaller.com/wiki/Asterisk_on_EC2

# MySQL Replication and Clustering #
  * [How to: Move / Copy a live MySQL database (and what not to do)](http://www.softwareprojects.com/resources/programming/t-how-to-move-copy-a-live-mysql-database-1257.html)
  * [MySQL Failover Strategy using State Management](http://dev.mysql.com/tech-resources/articles/failover-strategy-part1.html)
  * [MySQL Cluster Carrier Grade Edition](http://www.mysql.com/industry/telecom/)
  * [High Availability and Scalability](http://dev.mysql.com/doc/refman/5.0/en/ha-overview.html)
  * [An introduction to replication](http://www.databasejournal.com/features/mysql/article.php/3355201)
  * [Restoring lost data from the Binary Update Log](http://www.databasejournal.com/features/mysql/article.php/3326351)
  * [Live Backups of MySQL Using Replication](http://www.onlamp.com/pub/a/onlamp/2005/06/16/MySQLian.html)

## Tcpdump ##
```
   tcpdump -s 0 -i eth0 src 10.98.10.14 -vvv -ttt -e -w feb7-8.49am.pbx2.pcap
```

## CPAN ##
http://sial.org/howto/perl/life-with-cpan/
http://www.perlmonks.org/?node_id=394737

# Links on how to create live CDs #
Investigate creating ISO images for releases.

  * http://www-128.ibm.com/developerworks/linux/library/l-fedora-livecd/index.html
  * http://fedoraproject.org/wiki/FedoraLiveCD
  * http://fedoraproject.org/wiki/FedoraLiveCD/LiveCDHowTo
  * http://www.byteclub.net/wiki/CentOS_Live
  * https://projects.centos.org/trac/livecd/wiki
  * https://webapps.saugus.k12.ca.us/community/jklein/weblog/450.html
  * http://sipx-wiki.calivia.com/index.php/A_Kickstart_CD_for_sipX_on_CentOS
  * http://www.linux-live.org/
  * http://www.techonthenet.com/linux/centos4_update.php

# Virtualization #
  * [Using Xen for High Availability Clusters](http://www.onlamp.com/pub/a/onlamp/2008/02/05/using-xen-for-high-availabilty-clusters.html)
  * [Building A Virtual Cluster with Xen (Part One)](http://www.clustermonkey.net//content/view/139/33/)
  * [ztxen a zaptel timer for XEN virtual domains](http://lists.digium.com/pipermail/asterisk-dev/2007-May/027470.html)
  * [Example PCI hiding in dom0 and direct access in domU](http://lists.xensource.com/archives/html/xen-users/2005-11/msg00656.html)
  * [loading ztdummy in xen creates an rtc problem](http://bugs.digium.com/view.php?id=9592)
  * [Build your own Asterisk@Home Xen domU](http://code.blitzaffe.com/pages/home/entry/11_build_your_own_asterisk_home)
  * [Fedora Core 6, Xen and Asterisk](http://www.crc.id.au/fedora-core-6-xen-and-asterisk/)

## Fax ##
  * [Faxterisk](http://www.attractel.com/t38.html)
  * [Receiving and sending faxes with asterisk 1.4, NVFaxDetect, iaxmodem and hylafax](http://blogtech.oc9.com/index.php?option=com_content&view=article&catid=4:asterisk&id=77:20071121ast&Itemid=6)
  * [T.38](http://www.voip-info.org/wiki/view/T.38)
  * [T.38 Fax Support, Hype or Reality?](http://blog.tmcnet.com/blog/tom-keating/voip/t38-fax-support-hype-or-reality.asp)
  * [AsterFax - Asterisk Fax](http://asterfax.sourceforge.net/)
  * [HylaFax and IAXmodem Installatioin](http://aussievoip.com.au/wiki/HylaFax+and+IAXmodem+Installatioin)
  * [IAXmodem](http://iaxmodem.sourceforge.net/)
  * [NVFaxDetect](http://www.voip-info.org/wiki-NVFaxDetect)
  * [What is AvantFAX?](http://www.avantfax.com/)
  * [Build A HylaFax Server With AvantFax Frontend Using Debian Etch](http://www.howtoforge.com/build-a-hylafax-server-with-avantfax-on-debian-etch)
  * [Fax Services with Hylafax](http://www.linux.org/lessons/advanced/x833.html)
  * [How to build a Hylafax server (i.e. Hylamonster)](http://howtoforge.com/linux_hylafax_server)
  * [How To Set Up A Debian Linux Fax Server](http://www.aboutdebian.com/fax.htm)
  * [Winprint HylaFAX](http://winprinthylafax.sourceforge.net/)
  * [Asterisk and fax calls](http://www.voip-info.org/wiki-Asterisk+fax)
  * [Faxing with Asterisk](http://scottstuff.net/blog/articles/2004/03/28/faxing-with-asterisk)
  * [T.38 Passthrough](http://www.voip-info.org/tiki-index.php?page=Asterisk%20T.38)
  * [Avantfax](http://www.avantfax.com/)
  * [Callweaver](http://www.callweaver.org/blog?category=t.38)
  * [Fax Gateway](http://bugs.digium.com/view.php?id=13405)

## FreeSWITCH ##
  * http://www.voiceovergurl.com/
  * [PBX prompts](http://www.pbxprompts.com/)
  * http://www.gmvoices.com/
  * http://lists.freeswitch.org/pipermail/freeswitch-users/2008-January/001932.html
  * http://lists.freeswitch.org/pipermail/freeswitch-users/2008-July/004178.html
  * http://wiki.freeswitch.org/wiki/FreeSwitch_Scheduler_API
  * http://wiki.freeswitch.org/index.php?title=Misc._Dialplan_Tools_sched_hangup

## Grails ##
  * [Grails services and Google Maps](http://www.ibm.com/developerworks/java/library/j-grails05208/index.html)
  * [Masterisk Grails](http://www.ibm.com/developerworks/views/java/libraryview.jsp?search_by=mastering+grails)
  * http://fbflex.wordpress.com/2008/05/15/rest-service-xml-output-in-grails-via-content-negotiation/
  * [Grails, REST and JSecurity](http://www.grassr.com/wordpress/?p=18)
  * http://henyoonline.blogspot.com/2008/07/rest-with-grails.html

## Tools ##
### Diffs and Merging ###
  * [Meld](http://meld.sourceforge.net/index.html)
  * [Winmerge](http://www.winmerge.org/index.php)
  * [Tortoise Merge](http://tortoisesvn.tigris.org/TortoiseMerge.html)
  * [Beyond Compare](http://www.scootersoftware.com/)
  * http://www.hanselman.com/blog/ScottHanselmans2007UltimateDeveloperAndPowerUsersToolListForWindows.aspx
  * http://opensourceresources.wikispaces.com/

# Email/ Newsletter #
  * [Marketing](http://www.openemm.org/)
  * http://www.phplist.com/

# PDF #
  * http://www.primopdf.com/